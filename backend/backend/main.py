...
def suggest(skills):
    ...
    if len(valid_indices) == 0:
        ...
        # ✅ FIX: Check if we have enough indices for slicing [5:9]
        sorted_indices = valid_indices[np.argsort(similarities[valid_indices])[::-1]]
        if len(sorted_indices) <= 5:
            print("⚠️  DEBUG: Not enough matches, using all available indices")
            top_indices = sorted_indices  # Use all available
        else:
            top_indices = sorted_indices[5:9]  # Skip first 5, take next 4
    ...
    for i in recommended:
        ...
        for raw_word in words:
            # ✅ FIX: Strip unwanted characters BEFORE checking filter_words
            word = raw_word.strip(unwanted_chars)
            if word.lower() in filter_words:  # Also make it case-insensitive
                continue
    ...
    for i in range(len(recommended)):
        for j in recommended_skills[:]:  # Use slice to avoid modification during iteration
            if j in recommended[i]["skills"]:
                recommended[i]["suggested_skill"] = j
                recommended_skills.remove(j)
                break
                
    # ✅ FIX: Filter properly without modifying list during iteration
    final_recommended = []
    for item in recommended:
        if "suggested_skill" in item:  # Check if it has the suggested_skill key
...
def profile_setup():
    ...
    if not results:
        return jsonify({
            ...
            "recommendations": [],
            "partialMatches": partial_results  # ✅ Still include partial matches even if no main results
        })
    ...
    return jsonify({
        ...
        "partialMatches": partial_results  # ✅ Include partial matches
    })
